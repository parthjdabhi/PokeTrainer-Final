...
class SearchViewController: UIViewController {
    ...
    func getCurrentUserReference() {
        ...
        ref.child("users").child(userID!).child("profileData").child("gameDetails").observeEventType(.Value, withBlock: { (snapshot) in
            ...
            if let Dict = snapshot.valueInExportFormat() as? NSDictionary {
                if let isPairingFirstTime = Dict["isPairingFirstTime"] as? String where isPairingFirstTime == "true" {
                    //isPairingFirstTime
                    self.isPairingFirstTime = true
                    self.ref.child("users").child(userID!).child("profileData").child("gameDetails").setValue("false", forKey: "isPairingFirstTime")
                }
            }
...
func searchPeopleMatched() {
    ...
    private func isMatchedUser(snapshot:FIRDataSnapshot) -> Bool {
        ...
        if !self.isInNearBy(dic)
        {
            ...
            //if User is matching first time ignore game type matching
        else if (isPairingFirstTime == true) {
            return true
        }
        else
        {
    ...
    func showInviteView()
    {
        //Can invite people when not have any card
        self.viewInvite.hidden = false
    }
